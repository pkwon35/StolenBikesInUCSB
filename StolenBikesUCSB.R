...
bikesStolen <- read.csv("/Users/matthewpeterschmidt/Desktop/Stolen.Bikes.In.UCSB/bikesStolenUCSB.csv", header = TRUE)
...
bikeParking <- read.csv("/Users/matthewpeterschmidt/Desktop/StolenBikesInUCSB-eri_branch/bikeParking.csv", header = TRUE)
#bikesStolen[,1] <- factor(bikesStolen[,1], levels=levels(bikeParking[,1]))
for (i in 1:1326) {
  for (j in 1:45) {
...
bargraph <- ggplot(data = density, aes (x = Location, y = count)) +
  geom_col(col = 5) + 
  xlab("Location") + ylab("count") + ggtitle("Frequency of Stolen Bikes") +
  coord_flip()
